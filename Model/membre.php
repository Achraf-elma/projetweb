...
#Resultat: int correspondant a l'idMembre de la membre dont le nom est en parametre
{
 ...
 #Resultat: int correspondant a l'idMembre de la membre dont le nom est en parametre
 {
   ...
   $result = $bd->query("SELECT (membre.idmembre) AS ID,pseudo,email, telephone, nomVille, positive,neutre, negatif, description FROM membre, ville, profil WHERE ville.idville = membre.idville AND profil.idmembre = membre.idmembre AND pseudo ='".$pseudo."'");
   $Membre=$result->fetch();
